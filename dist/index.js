...
app.post('/api/knowledge/add', async (c) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(name || content?.substring(0, 20) || '未命名', chinese_name || '', category || '未分类', summary || content?.substring(0, 100) || '', content || '', tags || '', links || '', new Date().toISOString(), new Date().toISOString()).run();
